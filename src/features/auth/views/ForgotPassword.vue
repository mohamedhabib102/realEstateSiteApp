...
import { Globe, ArrowLeft, MailCheck } from 'lucide-vue-next'
...
const toggleLanguage = () => {
    ...
    router.push(`/${newLocale}/forgot-password`)
}
...
const handleSubmit = () => {
    ...
    if (!email.value) {
        errorMessage.value = locale.value === 'ar' ? 'يرجى إدخال بريدك الإلكتروني' : 'Please enter your email'
        return
    }
    ...
    setTimeout(() => {
        ...
        successMessage.value = locale.value === 'ar' ? 'تم إرسال رابط إعادة التعيين بنجاح!' : 'Reset link sent successfully!'
                ...
                <div>
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-4">
                        ...
                        >
                            ...
                            <span v-else>{{ locale === 'ar' ? 'جارٍ الإرسال...' : 'Sending...' }}</span>
                        </button>
                    </form>

                    <!-- Info Note -->
                    <div class="mt-6 p-3.5 bg-primary/5 border border-primary/10 rounded-xl flex items-start gap-2.5">
                        <MailCheck class="w-4 h-4 text-primary shrink-0 mt-0.5" />

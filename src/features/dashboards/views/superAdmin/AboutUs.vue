...
import { Save, Info, CheckCircle2 } from 'lucide-vue-next'
import { aboutUsEditable } from '../../../../data/superAdminDashboard'

const { locale } = useI18n()

const fields = ref(aboutUsEditable.map((f) => ({ ...f })))
const toast = ref('')
    ...
    <div class="space-y-6">
        <div class="flex flex-wrap items-end justify-between gap-3">
            <div v-motion :initial="{ opacity: 0, y: 12 }" :enter="{ opacity: 1, y: 0, transition: { duration: 400 } }">
                ...
                <p class="text-sm text-gray-500 mt-1">{{ locale === 'ar' ? 'تعديل محتوى صفحة من نحن' : 'Edit the About Us page content' }}</p>
            </div>
        ...
        <section v-motion :initial="{ opacity: 0, y: 14 }" :enter="{ opacity: 1, y: 0, transition: { duration: 450, delay: 120 } }" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h3 class="font-bold text-gray-900 flex items-center gap-2 mb-5"><Info class="w-5 h-5 text-primary" />{{ locale === 'ar' ? 'محتوى الصفحة' : 'Page Content' }}</h3>
            <div class="space-y-5">
                <div v-for="f in fields" :key="f.key">
                    <label class="block text-xs font-semibold text-gray-700 mb-2">{{ locale === 'ar' ? f.label.ar : f.label.en }}</label>
                    <textarea v-model="f.value" rows="3" class="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none"></textarea>
                </div>
            </div>
        </section>

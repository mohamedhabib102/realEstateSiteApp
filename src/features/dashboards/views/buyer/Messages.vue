...
import { buyerMessages } from '../../../../data/buyerDashboard'

const { locale } = useI18n()

const search = ref('')
const activeId = ref<string | null>(buyerMessages[0]?.id ?? null)
const draft = ref('')

const filtered = computed(() =>
    buyerMessages.filter((m) =>
        (locale.value === 'ar' ? m.ownerAr : m.ownerEn).toLowerCase().includes(search.value.toLowerCase())
    )
)

const active = computed(() => buyerMessages.find((m) => m.id === activeId.value) ?? null)
...
<template>
    <div class="space-y-6">
        ...
        <div v-motion :initial="{ opacity: 0, y: 12 }" :enter="{ opacity: 1, y: 0, transition: { duration: 400 } }">
            <h1 class="text-2xl font-bold text-gray-900">{{ locale === 'ar' ? 'الرسائل' : 'Messages' }}</h1>
            <p class="text-sm text-gray-500 mt-1">{{ buyerMessages.filter((m) => m.unread > 0).length }} {{ locale === 'ar' ? 'رسالة غير مقروءة' : 'unread messages' }}</p>
        </div>
        ...
        <div v-motion :initial="{ opacity: 0, y: 14 }" :enter="{ opacity: 1, y: 0, transition: { duration: 450, delay: 100 } }"
            ...
            <aside class="border-gray-100 md:border-r lg:border-gray-100" :class="locale === 'ar' ? 'md:border-l md:border-r-0' : 'md:border-r'">
                <div class="p-4 border-b border-gray-100">
                    <div class="relative">
                        ...
                        <input
                            v-model="search"
                            type="text"
                            :placeholder="locale === 'ar' ? 'ابحث عن مالك...' : 'Search owners...'"
                ...
                <div class="divide-y divide-gray-50">
                    <button
                        ...
                        :class="activeId === m.id ? 'bg-primary/5' : ''"
                    >
                        <div class="flex items-center gap-3">
                            <div class="relative shrink-0">
                                <img :src="m.avatar" alt="" class="w-11 h-11 rounded-full object-cover" />
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between">
                                    <span class="font-semibold text-gray-900 text-sm">{{ locale === 'ar' ? m.ownerAr : m.ownerEn }}</span>
                                    <span class="text-[11px] text-gray-400 shrink-0">{{ m.time }}</span>
                                </div>
                                <div class="flex items-center justify-between mt-0.5">
                                    <span class="text-xs text-gray-500 truncate">{{ locale === 'ar' ? m.lastMessageAr : m.lastMessageEn }}</span>
                                    <span v-if="m.unread" class="w-4.5 h-4.5 min-w-[18px] px-1.5 bg-secondary text-white text-[10px] font-bold rounded-full flex items-center justify-center shrink-0 ms-2">{{ m.unread }}</span>
                                </div>
                            </div>
                        </div>
                    </button>
            ...
            <section v-if="active">
                <div class="p-4 border-b border-gray-100 flex items-center gap-3">
                    ...
                    <div>
                        <div class="font-bold text-gray-900 text-sm">{{ locale === 'ar' ? active.ownerAr : active.ownerEn }}</div>
                        <div class="text-[11px] text-gray-400">{{ locale === 'ar' ? active.propertyAr : active.propertyEn }}</div>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-4 bg-[#FAFBFD]">
                    ...
                    <div class="flex justify-end">
                        <div class="max-w-[75%] bg-primary text-white rounded-2xl rounded-tr-sm px-4 py-2.5 text-sm shadow-sm">
                            {{ locale === 'ar' ? 'أنا مهتم بالعقار، هل يمكنني حجز معاينة؟' : 'I am interested, could I book a viewing?' }}
                            <span class="block text-[10px] text-white/70 text-end mt-1">1h</span>
                        </div>
                    </div>
                    <div class="flex justify-start">
                        <div class="max-w-[75%] bg-white border border-gray-100 rounded-2xl rounded-tl-sm px-4 py-2.5 text-sm text-gray-700 shadow-sm">
                            {{ locale === 'ar' ? 'بالتأكيد، يسعدني الترتيب معك.' : 'Of course, happy to arrange it with you.' }}
            ...
            <section v-else class="flex flex-col items-center justify-center p-10 text-gray-400">
                ...
                <p class="font-medium">{{ locale === 'ar' ? 'اختر رسالة لعرضها' : 'Select a message to view' }}</p>
            </section>
        </div>
    </div>
</template>